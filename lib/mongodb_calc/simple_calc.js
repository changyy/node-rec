...
function mongodb_build_user_prefer_via_user_item_pair_mapreduce(server, collection_raw, collection_item, collections_matrix, collection_out, callback) {
	...
	conn.once('open', function () {
		var o = {};
		o.scope = { level1_in_field: '_id' };
		o.verbose = true;
		o.jsMode = true;
		o.map = function(){
			if (this[level1_in_field]) {
				//var key = this[level1_in_field][i]+"-"+this[input_field][j];
				//emit(key, 1);
			}
		}
		o.reduce = function(k, vals) {
		...
		var model = conn.model( 'rec', exports.uniq_target_schema, collection_item);
		var moment = require('moment');
		model.mapReduce(o, function (err, results) {
			if(err)
				console.log(err);
			...
			if (callback) 
				callback(
					'['+moment().format() +'], out: ' + collection_out
				);
		});
