...
function mongodb_build_item_based_co_occurrence_matrix_prepare(server, collection_in, collection_in_item_field, collection_out, callback) {
	...
	conn.once('open', function () {
		...
		stream.on('data', function (doc) {
			stream.pause();
			console.log('get doc:'+ doc[collection_in_item_field].length);
			var output_item = [];
			...
			function handleRecordPair(i, j, max) {
				...
				if (output_item.length >= db_bulk_load_cnt) {
					...
					target.collection.insert(output_item, {w:1}, function (err) {
						console.log("\tbuild_item_based_co_occurrence_matrix_prepare\t"+moment().format()+"\tinsert:"+output_item.length+", at ("+i+","+j+"), max:"+max);
						if (err)
							console.log(err);
						output_item = [];
						// RangeError: Maximum call stack size exceeded
			...
			function handleFinish() {
				if (output_item.length) {
					var target = conn.model( 'target', exports.item_based_co_occurrence_matrix_init_schema, collection_out);
					target.collection.insert(output_item, {w:1}, function (err) {
						console.log("\tbuild_item_based_co_occurrence_matrix_prepare\t"+moment().format()+"\tfinish insert:"+output_item.length);
						if (err)
							console.log(err);
						stream.resume();
						output_item = [];
		...
		if(!err) {
			...
			db.createCollection(collection_out, {w:1}, function(err, output) {
				...
				cursor.on('data', function(doc) {
					...
					if (callback) 
						callback(
							'['+moment().format() +'] '+ collection_in_key_field+"-"+ collection_in_value_field + ':\t' + count
						);
				});
			});
		}
				...
				model_part3.findOne( { _id: key }, function (err, doc3) {
					...
					if (doc3 && doc3.value) {
						...
						stream_user.on('data', function(user_info) {
							...
							//model_raw.findOne({ _id: {user: user_info._id, item:doc_item_part2._id}} , function (err, user_item) {
							//model_raw.findOne({ _id: {user: user_info._id, item:doc_item_part2._id}} , function (err, user_item) {
							//model_raw.find(function (err, user_item) {
								if (user_item) {
