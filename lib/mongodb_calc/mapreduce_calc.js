...
function mongodb_build_item_based_co_occurrence_matrix(server, collection_in, collection_in_item_field, collection_out, callback) {
	...
	collection_user_item = 'user_item';
	collection_user_item_filter = 'user_item_filter';
	collection_co_matrix_prepare = 'co_matrix_prepare';
	...
	conn.once('open', function () {
		...
		var max_co_matrix_item_count = 2048;
...
//print(JSON.stringify(this), target_user);
							...
							user_item_stream2.on('data', function(doc2) {
								...
								if (co_matrix_item.length > 0) {
									...
									db_import.bulk_load(server, collection_co_matrix_prepare, co_matrix_item, function(){
										...
										var model_co_matrix_prepare = sub_conn2.model( 'co_matrix_prepare', exports.schema_item_based_co_occurrence_matrix_prepare, collection_co_matrix_prepare);
			...
			conn.close();
			if (callback) 
				callback('['+moment().format() +'] out: ' + collection_out);
		});
	});
// */
/*
