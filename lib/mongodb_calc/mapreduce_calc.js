...
exports.schema_matrix_multiplication_prepare = new mongoose.Schema({ _id: {}, value: Number });
...
function mongodb_build_user_prefer_via_user_item_pair(server, collection_user, collection_user_item, collections_matrix, collection_out_prepare, collection_out, callback) {
	var conn = mongoose.createConnection('mongodb://'+server);
	conn.on('error', console.error.bind(console, 'connection error:'));
	conn.once('open', function () {
		var moment = require('moment');
		...
		var stream = model_user.find().stream();
		var user = [];
		...
		o.map = function(){
			if (
				...
				for (var i in user) {
					...
					key[in_matrix_row] = this['_id'][in_matrix_row];
					key[in_matrix_column] = this['_id'][in_matrix_column];
		...
		stream.on('data', function(doc) {
			...
			conn = mongoose.createConnection('mongodb://'+server);
			conn.on('error', console.error.bind(console, 'connection error:'));
			conn.once('open', function () {
				o.scope = { 
				...
				o.map = function(){
					if (
						...
						var key = { 
							t: out_matrix_type,
							u: this['_id'][in_matrix_column],
							i: this['_id'][in_matrix_row]
						};
						emit(key, this[in_matrix_value]);
					}
				}
				...
				model_user_item_value.mapReduce(o, function (err, results) {
					...
					var o = {};
					o.scope = { 
						matrix_typeFieldName: 't',
						matrix_userFieldName: 'u',
						matrix_itemFieldName: 'i',
						matrix_rowFieldName: 'r',
						matrix_columnFieldName: 'c',
					...
					o.map = function() {
						if (
							this['_id'] != undefined
							&& undefined != this['_id'][matrix_typeFieldName] 
							&& undefined != this['_id'][matrix_userFieldName] 
							&& undefined != this[matrix_valueFieldName]
						) {
							if (this['_id'][matrix_typeFieldName] == 'left') {
								if (undefined != this['_id'][matrix_rowFieldName] && undefined != this['_id'][matrix_columnFieldName]) {
									var key = {}
									key[matrix_userFieldName] = this['_id'][matrix_userFieldName];
									key[matrix_itemFieldName] = this['_id'][matrix_rowFieldName];
									var val = {}
									val[matrix_valueFieldName] = this[matrix_valueFieldName];
									val[matrix_indexFieldName] = this['_id'][matrix_columnFieldName];
									emit(key, val);
								}
							} else if (this['_id'][matrix_typeFieldName] == 'right') {
								if (undefined != this['_id'][matrix_itemFieldName]) {
									var key = {}
									key[matrix_userFieldName] = this['_id'][matrix_userFieldName];
									key[matrix_itemFieldName] = this['_id'][matrix_itemFieldName];
									var val = {}
									val[matrix_valueFieldName] = this[matrix_valueFieldName];
									val[matrix_indexFieldName] = this['_id'][matrix_itemFieldName];
									emit(key, val);
								}
							}
						}
...
print(JSON.stringify(vals));
					...
					model_prepare = conn.model( 'matrix_prepare', exports.schema_matrix_multiplication_prepare, collection_out_prepare);
					model_prepare.mapReduce(o, function(err, results) {
						...
						conn.close();
						if (callback) 
							callback('['+moment().format()+'] total user: ' + total_user);
					});
				});
			});
		});
